...
export default function TiposLugar({ showHeader = true }: { showHeader?: boolean }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setData(await tipoLugarService.getAll());
    } catch {
      setData([]);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: number | string) => {
    ...
    try {
      await tipoLugarService.remove(id);
    ...
    <div>
      {showHeader ? (
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-100">Tipos de Lugar</h1>
            <p className="text-sm text-slate-450 mt-0.5">Administrar categorías de ubicaciones</p>
          </div>
          <button
          ...
          >
            <FiPlus size={16} /> Nuevo Tipo de Lugar
          </button>
        </div>
      ) : (
      ...
      <DataTable<TipoLugar>
        ...
        searchPlaceholder="Buscar tipo de lugar..."
      />
          ...
          <div className="bg-slate-900 rounded-2xl w-full max-w-md border border-white/5 shadow-2xl overflow-hidden relative transform transition-transform duration-300 scale-100">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-slate-950/20">
              <h2 className="text-lg font-bold text-slate-100">
                {editId ? "Editar Tipo de Lugar" : "Registrar Nuevo Tipo de Lugar"}
              </h2>
              <button
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej. Aula, Laboratorio, Oficina, Auditorio"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ej. Espacios destinados a clases o laboratorios"
                  className="w-full px-3.5 py-2.5 bg-slate-950/50 border border-white/10 rounded-xl text-sm text-white placeholder-white/20 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all"
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
                ...
                >
                  {saving ? "Guardando..." : editId ? "Guardar Cambios" : "Crear Tipo de Lugar"}
                </button>
              </div>
            </form>
          </div>
