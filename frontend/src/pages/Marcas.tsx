...
export default function Marcas() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setData(await marcaService.getAll()); } catch { setData([]); } finally { setLoading(false); }
  }, []);
  ...
  const handleDelete = async (id: number | string) => {
    ...
    try { await marcaService.remove(id); setData((prev) => prev.filter((d) => d.id !== id)); }
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-bold text-slate-800">Marcas</h1>
        <p className="text-sm text-slate-500 mt-0.5">Administrar marcas de activos</p>
      </div>
